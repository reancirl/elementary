            ...
            <form action="{{ route('school-years.update', $schoolYear->id) }}" method="post">
                ...
                <div class="modal-body">
                    <select name="year_start" class="form-control mb-3" id="year_start" required>
                        ...
                        @for ($i = 2020; $i <= 2030; $i++)
                            <option value="{{ $i }}" {{ $i == $schoolYear->year_start ? 'selected' : '' }}>
                                {{ $i }}</option>
                        @endfor
                    </select>
                    <select name="year_end" class="form-control mb-3" id="year_end" required>
                        ...
                        @for ($i = 2021; $i <= 2031; $i++)
                            <option value="{{ $i }}" {{ $i == $schoolYear->year_end ? 'selected' : '' }}>
                                {{ $i }}</option>
                        @endfor
                    </select>
                    <select name="status" class="form-control mb-3" id="status" required>
                        <option value="0" {{ $schoolYear->status == 0 ? 'selected' : '' }}>Inactive</option>
